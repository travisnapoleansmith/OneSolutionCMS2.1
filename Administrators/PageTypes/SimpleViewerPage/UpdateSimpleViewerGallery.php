<?php
	...
	*/
	...
	if ($ReferPageID === 'PageID=223') {
		$LogPage = FALSE;
		
		if ($LogPage === TRUE) {
			$LogFile = "UpdateSimpleViewerGallery.txt";
			$LogFileHandle = fopen($LogFile, 'a');
			$FileInformation = 'Logging - Update SimpleViewer Gallery Script Loading - ' . date("F j, Y, g:i a") . "\n";
		...
		$PageName .= $_POST['UpdateSimpleViewerGallery'];
	
		$FileLocation = 'TEMPFILES/';
		
		$TempGallery = array();
		$Gallery = array();
		$UpdateLookupData = array();
		
		foreach ($_POST as $Key => $Value) {
			if ($Key !== 'UpdateSimpleViewerGallery') {
				if ($Key !== 'ImageLocation') {
					if (strstr($Key, "Image")) {
						$TempGallery[$Key] = $Value;
						$UpdateLookupData[$Key] = $Value;
					}
				}
			}
		}
		...
		$hold = $Tier6Databases->FormSubmitValidate('UpdateSimpleViewerGallery', $PageName, $FileLocation, $Gallery, 'image', $UpdateLookupData, $XMLOptions);
		...
		if ($hold) {
			...
			if ($LogPage === TRUE) {
				$LogFile = "UpdateSimpleViewerGallery.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Update SimpleViewer Gallery Top Script - ' . $NewPageID . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
			...
			$GalleryID = $hold['FilteredInput']['GalleryID'];
			...
			$NewSimpleViewerGallery = $GalleryID;
			...
			*/
			...
			// SUBMIT SIMPLEVIEWER DATA - NEEDS WORK
			...
			$OldPageID = array();
			$OldPageID['PageID'] = $NewSimpleViewerGallery;
			...
			$OldPageID['OBJECT'] = $FlashObject;
			
			$Tier6Databases->ModulePass('XhtmlSimpleViewer', 'simpleviewer', 'updateFlash', $OldPageID);
			$Tier6Databases->ModulePass('XhtmlSimpleViewer', 'simpleviewer', 'createFlash', $Flash);
			
			unset($OldPageID['OBJECT']);
			...
			$SimpleViewerObject->updateLookupGallery($OldPageID);
			...
			$SimpleViewerObject->updateGallery($OldPageID);
			$SimpleViewerObject->createGallery($SimpleViewerGallery);
			
			
			// SUBMIT FORM DATA
			$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormOption', array('PageID' => array('PageID' => $UpdateSimpleViewerGallerySelect, 'ObjectID' => $GalleryID), 'Content' => $FormOption));
	
			$DeleteSimpleViewerGallery = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyDeleteSelectPage']['SettingAttribute'];
	
			$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormOption', array('PageID' => array('PageID' => $DeleteSimpleViewerGallery, 'ObjectID' => $GalleryID), 'Content' => $FormOption));
			...
			$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormOption', array('PageID' => array('PageID' => $EnableDisableStatusChangeSimpleViewerGallery, 'ObjectID' => $GalleryID), 'Content' => $FormOption));
			
			
			$SimpleViewerGalleyCreatedUpdatePage = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyCreatedUpdatePage']['SettingAttribute'];
			
			if ($LogPage === TRUE) {
				$LogFile = "UpdateSimpleViewerGallery.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Update SimpleViewer Gallery Bottom Script - ' . $NewSimpleViewerGallery . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
			...
			header("Location: $SimpleViewerGalleyCreatedUpdatePage&GalleryID=$NewSimpleViewerGallery");
			exit;
		} else {
