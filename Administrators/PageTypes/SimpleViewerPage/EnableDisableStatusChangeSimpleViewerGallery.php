<?php
	...
	*/
	...
	if (!is_null($GalleryID)) {
		...
		$Flash['EnableDisable'] = $_POST['EnableDisable'];
		$Flash['Status'] = $_POST['Status'];
		...
		$Tier6Databases->ModulePass('XhtmlSimpleViewer', 'simpleviewer', 'updateFlashStatus', $Flash);
		unset($Flash['OBJECT']);
		unset($FlashObject);
		...
		$SimpleViewerObject->updateLookupGalleryStatus($Flash);
		
		$SimpleViewerObject->FetchDatabase ($PageID);
		
		$SimpleViewerObject->updateGalleryStatus($Flash);
		
		$FormOptionID = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyUpdateSelectPage']['SettingAttribute'];
		$SimpleViewerGalleryPageID = array();
		$SimpleViewerGalleryPageID['PageID'] = &$FormOptionID;
		$SimpleViewerGalleryPageID['ObjectID'] = $FormOptionObjectID;
		$SimpleViewerGalleryPageID['EnableDisable'] = $_POST['EnableDisable'];
		$SimpleViewerGalleryPageID['Status'] = $_POST['Status'];
		
		$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormOptionStatus', $SimpleViewerGalleryPageID);
		$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormSelectStatus', $SimpleViewerGalleryPageID);

		$FormOptionID = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyDeleteSelectPage']['SettingAttribute'];
		$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormOptionStatus', $SimpleViewerGalleryPageID);
		$Tier6Databases->ModulePass('XhtmlForm', 'form', 'updateFormSelectStatus', $SimpleViewerGalleryPageID);

		$SimpleViewerGalleyEnableDisablePage = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyEnableDisablePage']['SettingAttribute'];
		header("Location: $SimpleViewerGalleyEnableDisablePage");
		exit;
		
	} else {
		$SimpleViewerGalleyDeleteSelectPage = $Options['XhtmlSimpleViewer']['simpleviewer']['SimpleViewerGalleyEnableDisableSelectPage']['SettingAttribute'];
		header("Location: ../../index.php?PageID=$SimpleViewerGalleyEnableDisableSelectPage");
	}
