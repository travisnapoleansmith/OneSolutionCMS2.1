<?php
	...
	*/
	...
	if ($ReferPageID === 'PageID=110') {
		$LogPage = FALSE;
		
		if ($LogPage === TRUE) {
			$LogFile = "AddNewsPage.txt";
			$LogFileHandle = fopen($LogFile, 'a');
			$FileInformation = 'Logging - Add News Page Script Loading - ' . date("F j, Y, g:i a") . "\n";
		...
		if ($hold) {
			...
			if ($LogPage === TRUE) {
				$LogFile = "AddNewsPage.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Add News Page Top Script - ' . $NewPageID . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
			...
			if ($LogPage === TRUE) {
				$LogFile = "AddNewsPage.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Add News Page Bottom Script - ' . $NewPageID . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
				fwrite($LogFileHandle, print_r($NewsPageCreatedPage, TRUE));
				fwrite($LogFileHandle, "\n---------------------------------------------\n\n");
				fclose($LogFileHandle);
			}
