<?php
	...
	*/
	...
	if ($ReferPageID === 'PageID=130') {
		$LogPage = FALSE;
		
		if ($LogPage === TRUE) {
			$LogFile = "AddCalendarPage.txt";
			$LogFileHandle = fopen($LogFile, 'a');
			$FileInformation = 'Logging - Add Calendar Page Script Loading - ' . date("F j, Y, g:i a") . "\n";
		...
		if ($hold) {
			...
			if ($LogPage === TRUE) {
				$LogFile = "AddCalendarPage.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Add Calendar Page Top Script - ' . $NewPageID . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
			...
			if ($LogPage === TRUE) {
				$LogFile = "AddCalendarPage.txt";
				$LogFileHandle = fopen($LogFile, 'a');
				$FileInformation = 'Logging - Add Calendar Page Bottom Script - ' . $NewPageID . ' - ' . date("F j, Y, g:i a") . "\n";
				fwrite($LogFileHandle, $FileInformation);
				fwrite($LogFileHandle, print_r($CalendarPageCreatedPage, TRUE));
				fwrite($LogFileHandle, "\n---------------------------------------------\n\n");
				fclose($LogFileHandle);
			}
