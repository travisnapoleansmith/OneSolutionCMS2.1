	...
	if ($RefererPage == $ServerLocation . "/Administrators/Panel/Managers/StatsManager/StatsManager.php") {
		...
		if (is_null($Format) === TRUE) {
			...
			if (is_array($PageListings) === TRUE) {
				$FileName = $GridStatsDataConfiguration['Tab Configuration Files']['IPAddresses'];
				if (file_exists($FileName) === TRUE) {
					$IPAddressesConfiguration = parse_ini_file($FileName, true);
				} else {
					$IPAddressesConfiguration = NULL;
				}
				
				$ColumnNames = array();
				buildColumnTemplate($IPAddressesConfiguration, $Elements);
				buildColumnNames($IPAddressesConfiguration, $ColumnNames);
