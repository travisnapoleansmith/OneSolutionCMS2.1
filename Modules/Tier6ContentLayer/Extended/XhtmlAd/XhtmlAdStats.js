...
function AdStats() {
	window.onload = function () {
		...
		var PageUrl = 'Modules/Tier6ContentLayer/Extended/XhtmlAd/XMLBrowserAdStats.php?PageID=' + PageID;
		
		var AdvertisingID = new Array();
		var AdvertisingTableName = new Array();
		...
		$('#side-ad-middle').children('div').each(function () {
			var CurrentID = $(this).attr('id');
			var CurrentIDArray = CurrentID.split('_');
			
			AdvertisingTableName.push(CurrentIDArray[1]);
			AdvertisingID.push(CurrentIDArray[3]);
			//alert(CurrentIDArray);
		});
		
		AdvertisingID = AdvertisingID.toString();
		
		AdvertisingTableName = AdvertisingTableName.toString();
		
		$.ajax({
			url: PageUrl,
			type: "POST", 
			data: { AdvertisingID: AdvertisingID, AdvertisingTableName: AdvertisingTableName,
					AdobeReaderVersion: PluginDetectAdobeReaderVersion, DevalvrVersion: PluginDetectDevalvrVersion, FlashVersion: PluginDetectFlashVersion,
